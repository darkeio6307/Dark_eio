...
async function loadNotices() {
  ...
  try {
    ...
    snap.docs.forEach((docItem, idx) => {
      ...
      html += `<div class="flex gap-3 items-start slide-in" style="animation-delay:${idx * 0.05}s"><div class="w-2 h-2 rounded-full mt-2 shrink-0 ${dotClass}" ${dotStyle}></div><div><p class="text-sm leading-relaxed content-pre-wrap" style="color:var(--app-text-muted)">${escapeHtml(n.body)}</p><p class="text-[11px] mt-1 font-medium" style="color:var(--app-text-dim)">${escapeHtml(dateStr)}</p></div></div>`;
...
function renderPost(post, postId) {
  ...
  const commentsHtml = (post._comments || []).map((c) => {
    const cAvatar = c.authorPhoto ? `<img src="${escapeHtml(c.authorPhoto)}" alt="">` : `<span>${escapeHtml((c.author || 'U').charAt(0))}</span>`;
    return `<div class="comment-item"><div class="comment-avatar">${cAvatar}</div><div class="comment-body"><p class="comment-author">${escapeHtml(c.author)}</p><p class="comment-text">${escapeHtml(c.text)}</p></div></div>`;
  }).join('');

  const commentInputHtml = state.user
    ? `<div class="comment-input-row"><input type="text" id="comment-input-${postId}" placeholder="Add a comment..." class="app-input" onkeydown="if(event.key==='Enter')App.addComment('${postId}')"><button type="button" onclick="App.addComment('${postId}')" class="app-btn" style="width:36px;padding:0;border-radius:50%"><i class="fa-solid fa-paper-plane text-xs"></i></button></div>`
    : '';

  return `<div class="feed-post p-5" id="post-${postId}"><div class="feed-post-header mb-3"><div class="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 ${isAdmin ? 'avatar-frame-founder' : ''}" style="background:var(--app-surface2)">${avatarHtml}</div><div class="flex-1 min-w-0"><div class="flex items-center gap-2 flex-wrap"><p class="text-sm font-bold" style="color:var(--app-text)">${authorName}</p>${adminBadgeHtml}</div><p class="text-[10px]" style="color:var(--app-text-dim)">${timeStr}</p></div></div>${textContent ? `<p class="text-sm leading-relaxed mb-3" style="color:var(--app-text-muted)">${textContent}</p>` : ''}${imageContent}<div class="flex items-center gap-4 pt-2 border-t" style="border-color:var(--app-border)"><button type="button" onclick="App.likePost('${postId}')" class="like-btn ${isLiked ? 'liked' : ''}" style="color:${likeColor}"><i class="${heartClass}"></i><span>${likeCount}</span></button><button type="button" onclick="App.toggleComments('${postId}')" class="like-btn" style="color:var(--app-text-dim)"><i class="fa-regular fa-comment"></i><span id="comment-count-${postId}">${commentCount}</span></button><span class="text-[11px] ml-auto" style="color:var(--app-text-dim)">${likeCount} likes</span></div><div class="comment-section hidden" id="comments-${postId}">${commentsHtml}${commentInputHtml}</div></div>`;
}
...
async function loadComments(postId) {
  ...
  try {
    ...
    if (!container) return;
    const inputEl = container.querySelector('.comment-input-row');
    let html = '';
    snap.docs.forEach((d) => {
      const c = d.data();
      const cAvatar = c.authorPhoto ? `<img src="${escapeHtml(c.authorPhoto)}" alt="">` : `<span>${escapeHtml((c.author || 'U').charAt(0))}</span>`;
      html += `<div class="comment-item"><div class="comment-avatar">${cAvatar}</div><div class="comment-body"><p class="comment-author">${escapeHtml(c.author)}</p><p class="comment-text">${escapeHtml(c.text)}</p></div></div>`;
    });
    if (snap.empty) html = '<p class="text-[11px] text-center py-2" style="color:var(--app-text-dim)">No comments yet. Be the first!</p>';
    if (inputEl) {
      const tempDiv = document.createElement('div');
      tempDiv.innerHTML = html;
      while (inputEl.previousSibling) inputEl.parentNode.removeChild(inputEl.previousSibling);
      container.insertBefore(tempDiv, inputEl);
    } else {
...
async function addComment(postId) {
  if (!state.user || !db) { showToast('Please login to comment', 'error'); return; }
  ...
  try {
    ...
    await db.collection('posts').doc(postId).collection('comments').add({
      text: text,
      author: state.profile?.name || state.user.displayName || 'Student',
...
async function viewStory(storyId) {
  ...
  try {
    ...
    const isVideo = s.mediaType === 'video' || (s.videoUrl && s.videoUrl.length > 0);
    if (isVideo) {
      // Show video
      if (img) img.classList.add('hidden');
      const vidEl = document.getElementById('story-viewer-vid');
      if (vidEl) { vidEl.src = s.videoUrl; vidEl.classList.remove('hidden'); vidEl.play(); }
    } else {
      // Show image
      const vidEl = document.getElementById('story-viewer-vid');
      if (vidEl) { vidEl.classList.add('hidden'); vidEl.pause(); vidEl.src = ''; }
      if (img) { img.src = s.imageUrl || ''; img.classList.remove('hidden'); }
    }
...
function closeStoryModal() {
  ...
  const img = $('#story-viewer-img');
  const vid = $('#story-viewer-vid');
  if (img) { img.src = ''; img.classList.remove('hidden'); }
  if (vid) { vid.pause(); vid.src = ''; vid.classList.add('hidden'); }
  if (state.storyTimeout) {
    clearTimeout(state.storyTimeout);
    state.storyTimeout = null;
  }
...
function openStoryCamera() {
  if (!state.user) { showToast('Please login to add story', 'error'); return; }
  const modal = $('#story-camera-modal');
  if (modal) modal.classList.add('open');
  state.storyFile = null;
  state.storyFileType = null;
  const preview = $('#story-preview');
  const previewImg = $('#story-preview-img');
  const previewVid = $('#story-preview-vid');
  const uploadBtn = $('#story-upload-btn');
  if (preview) preview.classList.add('hidden');
...
async function handleStoryFile(input, type) {
  const file = input.files[0];
  if (!file) return;

  // Validate size
  const maxSize = type === 'video' ? 30 * 1024 * 1024 : 5 * 1024 * 1024;
  if (file.size > maxSize) {
    showToast(`Max ${type} size: ${type === 'video' ? '30MB' : '5MB'}`, 'error');
    input.value = '';
    return;
  }
  ...
  if (type === 'video') {
    ...
    if (previewVid) { previewVid.src = objectUrl; previewVid.classList.remove('hidden'); }
  } else {
    if (previewVid) previewVid.classList.add('hidden');
    if (previewImg) { previewImg.src = objectUrl; previewImg.classList.remove('hidden'); }
  }
...
async function uploadStory() {
  if (!state.storyFile || !state.user || !db) {
  ...
  if (uploadBtn) {
    uploadBtn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i><span>Uploading...</span>';
    uploadBtn.disabled = true;
  }

  try {
    const file = state.storyFile;
    const type = state.storyFileType;

    // Upload to Cloudinary using unsigned upload preset
    const cloudName = 'dj2hlkplg';
    const uploadPreset = 'studypro_stories';
    ...
    formData.append('folder', 'studypro/stories');

    const response = await fetch(url, { method: 'POST', body: formData });
    const data = await response.json();

    if (!data.secure_url) {
      throw new Error(data.error?.message || 'Upload failed. Make sure unsigned upload preset "studypro_stories" is configured in your Cloudinary dashboard.');
    }
    ...
    if (uploadBtn) {
      uploadBtn.innerHTML = originalHtml || '<i class="fa-solid fa-paper-plane"></i><span>Publish Story</span>';
